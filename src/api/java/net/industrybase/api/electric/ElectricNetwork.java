...
public class ElectricNetwork {
    ...
    public int receiveFEEnergy(BlockPos pos, int maxReceive, boolean simulate) {
        ...
        if (!simulate)
            this.FEEnergy.add(root, receive);
        return receive;
    }
    ...
    public int extractFEEnergy(BlockPos pos, int maxExtract, boolean simulate) {
        ...
        if (!simulate)
            this.FEEnergy.remove(root, extract);
        return extract;
    }
            ...
            if (!this.wireConn.get(pos).isEmpty()) {
                ...
                while (iterator.hasNext()) {
                    ...
                    if (this.level.isAreaLoaded(another, 0)) {
                        this.level.getChunk(another).markUnsaved();
                    }
                }
                this.subscribes.get(pos)
                        .forEach(player -> PacketDistributor.sendToPlayer(player, new RemoveWiresPacket(pos)));
    ...
    public void removeWire(BlockPos from, BlockPos to) {
        this.tasks.offer(() -> {
            ...
            this.level.getChunk(from).markUnsaved(); // 不能检查区块是否加载
            this.level.getChunk(to).markUnsaved();
        });
    }
    ...
    public void cutWire(BlockPos from, BlockPos to) {
        if (this.wireConn.remove(from, to)) {
            ...
            this.spilt(from, to);
            this.subscribes.get(from)
                    .forEach(player -> PacketDistributor.sendToPlayer(player, new WireConnSyncPacket(from, to, true)));
            this.subscribes.get(to)
                    .forEach(player -> PacketDistributor.sendToPlayer(player, new WireConnSyncPacket(to, from, true)));
        }
    }
    ...
    private boolean hasFECapability(BlockPos pos, Direction side) {
        if (this.level.isAreaLoaded(pos, 0)) {
            ...
            if (blockEntity != null) {
                Level level = blockEntity.getLevel();
                if (level != null) {
                    return level.getCapability(Capabilities.Energy.BLOCK, pos, null, blockEntity, side) != null;
                }
            }
        }
        return false;
    ...
    private void linkWire(BlockPos from, BlockPos to) {
        ...
        if (this.wireConn.put(secondary, primary)) {
            ...
            this.link(primary, secondary);
            this.subscribes.get(from)
                    .forEach(player -> PacketDistributor.sendToPlayer(player, new WireConnSyncPacket(from, to, false)));
            this.subscribes.get(to)
                    .forEach(player -> PacketDistributor.sendToPlayer(player, new WireConnSyncPacket(to, from, false)));
        }
    }
        ...
        for (Map.Entry<BlockPos, Direction> entry : this.shuffle(this.FEMachines.entries())) {
            ...
            if (this.level.isAreaLoaded(target, 0)) {
                ...
                if (blockEntity != null) {
                    Level level = blockEntity.getLevel();
                    if (level != null) {
                        EnergyHandler capability = level.getCapability(Capabilities.Energy.BLOCK, target,
                        ...
                        if (capability != null) {
                            ...
                            try (var tx = Transaction.openRoot()) { // consume FE converted from EP first
                                int inserted = capability.insert(diff, tx);
                                forgeEnergy.remove(root, inserted);

                                if (forgeEnergy.count(root) <= 0) { // if all FE converted from EP have been consumed
                                    // then consume FE converted from FE
                                    int feReceived = capability.insert(FEDiff, tx);
                                    this.FEEnergy.remove(root, feReceived);
                                }
    ...
    public static class Manager {
        ...
        public static ElectricNetwork get(LevelAccessor level) {
            return INSTANCES.computeIfAbsent(Objects.requireNonNull(level, "Level can't be null!"),
                    ElectricNetwork::new);
        }
