...
import net.industrybase.capability.network.client.SubscribeWireConnPacket;
...
public class WireConnectableRenderer<T extends BlockEntity>
        implements BlockEntityRenderer<T, WireConnectableRenderState> {
    private @Nullable AABB aabb = null;
    ...
    @Override
    public WireConnectableRenderState createRenderState() {
        return new WireConnectableRenderState();
    }

    @Override
    public void extractRenderState(T blockEntity, WireConnectableRenderState state, float partialTicks,
            Vec3 cameraPosition,
        ...
        if (blockEntity instanceof IWireConnectable connectable) {
            if (blockEntity.hasLevel() && !connectable.isSubscribed()) {
                ClientPacketDistributor.sendToServer(new SubscribeWireConnPacket(blockEntity.getBlockPos()));
                connectable.setSubscribed();
            }
            state.wires = connectable.getWires();
        }
    }

    @Override
    public void submit(WireConnectableRenderState state, PoseStack poseStack, SubmitNodeCollector submitNodeCollector,
    ...
    private void renderWire(BlockPos from, BlockPos to, PoseStack.Pose pose, VertexConsumer buffer) {
        ...
        for (int i = 0; i <= 24; ++i) { // 一共 25 段
            addVertexPair(buffer, matrix4f, totalX, totalY, totalZ, horizonDistance, fromLightLevel, toLightLevel,
                    fromSkyLight,
    ...
    private static void addVertexPair(VertexConsumer consumer, Matrix4f matrix4f, float totalX, float totalY,
        ...
        consumer.addVertex(matrix4f, x - widthX, y + widthY, z + widthZ).setColor(red, green, blue, 1.0F)
