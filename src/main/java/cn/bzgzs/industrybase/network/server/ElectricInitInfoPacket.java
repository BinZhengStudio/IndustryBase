...
public class ElectricInitInfoPacket extends CustomPacket {
	private final Map<BlockPos, Collection<BlockPos>> wireConn;

	public ElectricInitInfoPacket(Map<BlockPos, Collection<BlockPos>> wireConn) {
		this.wireConn = wireConn;
	}

	public ElectricInitInfoPacket(FriendlyByteBuf buf) {
	...
	public void consumer(Supplier<NetworkEvent.Context> context) {
		context.get().enqueueWork(() -> Optional.ofNullable(Minecraft.getInstance().level).ifPresent(level -> ElectricNetwork.Manager.get(level).clientInitData(this.wireConn)));
