...
public class ElectricWireConnAddPacket extends CustomPacket {
	private final Map<BlockPos, Collection<BlockPos>> wireConn;

	public ElectricWireConnAddPacket(Map<BlockPos, Collection<BlockPos>> wireConn) {
		this.wireConn = wireConn;
	}

	public ElectricWireConnAddPacket(FriendlyByteBuf buf) {
	...
	public void consumer(Supplier<NetworkEvent.Context> context) {
		context.get().enqueueWork(() -> Optional.ofNullable(Minecraft.getInstance().level).ifPresent(level -> ElectricNetwork.Manager.get(level).addWireConn(this.wireConn)));
